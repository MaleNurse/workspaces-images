...
"
" Modified 08-Aug-2012 by Ron Record <rrecord@vmware> to detect Padrino apps
...
function! s:Detect(filename)
  ...
  if fn =~ '[\/]config[\/]apps\.rb$'
    return s:BufInit(strpart(fn,0,strlen(fn)-15))
  endif
  if isdirectory(fn)
  ...
  let ofn = ""
  while fn != ofn
    if filereadable(fn . "/config/environment.rb") || filereadable(fn . "/config/apps.rb")
      return s:BufInit(fn)
    endif
    let ofn = fn
    let fn = fnamemodify(ofn,':s?\(.*\)[\/]\(app\|config\|data\|db\|doc\|features\|lib\|log\|models\|public\|script\|spec\|stories\|test\|tmp\|vendor\)\($\|[\/].*$\)?\1?')
  endwhile
  return 0
endfunction
